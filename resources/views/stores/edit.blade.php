...
@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="mb-8">
            <h1 class="text-2xl font-bold text-gray-900">Edit Toko</h1>
            <p class="text-gray-600">Ubah data toko</p>
        </div>

        <div class="bg-white shadow rounded-lg">
            <form action="{{ route('stores.update', $store) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" id="code" name="code" required value="{{ old('code', $store->code) }}"
                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
                    ...
                    <div>
                        ...
                        <textarea id="address" name="address" rows="3"
                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
                            placeholder="Masukkan alamat lengkap toko">{{ old('address', $store->address) }}</textarea>
                        @error('address')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
    ...
    <!-- Modal Hapus Toko -->
    <div id="deleteStoreModal" x-data="{ show: false }" x-show="show" x-cloak
        class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" @click.self="show = false">
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white" @click.stop>
            <div class="mt-3">
                <h3 class="text-lg font-medium text-gray-900">Hapus Toko</h3>
                <p class="text-gray-600 mb-4">Apakah Anda yakin ingin menghapus toko ini? Tindakan ini tidak dapat
                    dibatalkan.</p>
    ...
    <script>
        function showDeleteModal() {
            const modal = document.getElementById('deleteStoreModal');
            const tryOpen = () => {
                if (modal && modal.__x) {
