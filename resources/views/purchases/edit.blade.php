...
@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="mb-8">
            ...
            <p class="text-gray-600">Ubah data purchase order</p>
        </div>

        <div class="bg-white shadow rounded-lg">
            <form action="{{ route('purchases.update', $purchase) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" value="{{ $purchase->supplier->name }}" readonly
                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm bg-gray-50 text-gray-500">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" value="{{ $purchase->store->name }}" readonly
                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm bg-gray-50 text-gray-500">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" id="expected_date" name="expected_date" required
                            value="{{ old('expected_date', $purchase->expected_date?->format('Y-m-d')) }}"
                    ...
                    <div>
                        ...
                        <div id="items-container">
                            @foreach ($purchase->items as $index => $item)
                                <div class="item-row flex gap-4 items-end mb-4 p-4 border border-gray-200 rounded-md">
                                    <div class="flex-1">
                                    ...
                                    <div class="w-32">
                                        ...
                                        <input type="number" name="items[{{ $index }}][unit_cost]"
                                            value="{{ $item->unit_cost }}" min="0" step="0.01" required
                                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                                    </div>
                                    <div class="w-16">
    ...
    @if ($purchase->status === 'pending')
        <!-- Modal Hapus Purchase Order -->
        <div id="deletePurchaseModal" x-data="{ show: false }" x-show="show" x-cloak
            class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" @click.self="show = false">
            <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white" @click.stop>
                <div class="mt-3">
                    <h3 class="text-lg font-medium text-gray-900">Hapus Purchase Order</h3>
                    <p class="text-gray-600 mb-4">Apakah Anda yakin ingin menghapus purchase order ini? Tindakan ini tidak
                        dapat dibatalkan.</p>
